...
The target capability is the Mirai/backdoor botnet capability family. Because the release's `Backdoor_Malware` class alone carries only 3,078 rows (3,218 in the per-attack representation) while the Section 30.20 program requires at least 2,000 reproduction-target, 1,000 verification-target and 500 screen-target examples in the gated roles, the declared target class is the union of the release's backdoor/botnet family: `BACKDOOR_MALWARE`, `MIRAI_GREETH_FLOOD`, `MIRAI_GREIP_FLOOD` and `MIRAI_UDPPLAIN`, which canonicalize to the single target class `BACKDOOR_MALWARE`. This is a declared capability-family mapping, not an accidental label collision, and it is the only exception to the collision rule below. The family supplies 2,524,809 rows, so every existing primary evidence minimum, role interval, sampling cap, statistical test, Holm family and material threshold applies to the secondary program unchanged. All other observed canonical attack labels plus benign are supported classes; the remaining three canonical labels become supported classes rather than a second target. The official CIC page documents 33 attacks in seven categories over a 105-device topology, explicitly lists `Backdoor malware`, and publishes statistics for 46 predictor features. These are validation expectations, not substitutes for inspecting the acquired CSV bytes. The observed raw CSV shards determine row counts, shard counts, exact file hashes, and the ordered predictor schema used by execution subject to the deterministic rules below.
...
Robustness poison-fraction sweep: **1%, 5%, 10%**, using the same replacement semantics. The configured sweep is the authoritative set of admissible source-backdoor poison fractions: the confirmatory fraction must be a member of it (validated when the configuration loads), and the implementation rejects any backdoor training step that declares a poison fraction outside the sweep rather than silently training at an undeclared strength.
...
The source candidate is the useful backdoored source exactly for the conditions that declare a compromised source-authority object — `Useful Backdoored Source — 5%`, `One/Two Source Copy`, `One Byzantine Source-Copy Reproducer` in the plurality, external-verification and Byzantine-bound reproducer families — and the clean legitimate target capability otherwise. Baselines keep their own mechanisms: methods other than `Resolved FedSIRA Core` perform no Section 7.1 opening and receive their source, or admit it, according to their own Section 16 contract.
...
The implementation is organized into layers with one-way dependencies:

* `datasets` — dataset discovery, schema, raw ingestion, preparation, partitioning, domain mapping, role/view materialization, dataset-specific transforms, and prepared-data adapters;
* `domain` — enums, semantic aliases, and domain models;
* `learning` — model definition, shared training mechanics, federated training, anchor training, post-reference/reproduction training, and parameter operations;
* `protocol` — FedSIRA method logic: admission, Capability Contract, proposal, reproduction, verification, synthesis, state rules, attacks, and baselines;
* `experiments` — executable experiment definitions, planning, prerequisite resolution, handler dispatch, execution, persistence, resolved-core derivation, and experiment lifecycle;
* `evaluation` — metrics, summaries, statistics, comparisons, and scientific evidence evaluation;
* `reporting` — tables, figures, protocol tables, completeness verification, and manuscript-facing export;
* `artifacts` — paths, semantic artifact identity, checksums, lifecycle, publication, atomic writes, reuse, and caching;
* `application`/`cli` — orchestration and a thin command adapter.

Concrete module filenames are an implementation detail and are not part of the scientific contract.
...
An artifact's **identity** is `SHA256` over the framed tuple `(family, instance, experiment, schema version, procedure identity, sorted (dependency name, dependency digest) pairs)`. Every declared dependency carries a **kind**: an *artifact* dependency names the identity of another published artifact and must resolve to a `Complete` publication, while a *content* dependency names an identity-defining digest (a model, view, population, rule, or numerical-runtime identity) that re-keys the artifact without being separately published. Verification therefore enforces resolution only for artifact dependencies; a content dependency is verified through the artifact's own identity and payload digest. Identity is therefore dependency-keyed: a change to any declared material dependency, to the producing procedure, or to the payload schema yields a new identity that cannot reuse the previous publication. The manifest additionally records the payload content digest and byte length, the lifecycle state, the producer, the declared dependencies with their digests, the authoritative configuration digest, and the repository revision for provenance. `Complete` is reached only by an atomic payload rename followed by an atomic manifest write and then an atomic `current.json` update; a torn publication leaves no readable artifact. Reading re-validates the schema version, the lifecycle state, the payload content digest and the payload length. Consumption re-validates content at the boundary as well: before a cell draws on prepared evidence, every prepared role view in that prepared root must have a published `Complete` artifact whose declared payload length, payload digest, and row count match the parquet payload and its sidecar. An unpublished, truncated, or altered view makes the cell `Invalid` with a prepare-phase invariant failure rather than being consumed silently; the check is memoized per process on the published identity, payload length, and modification time so it re-hashes only when a view actually changes. A manifest that cannot be parsed, including one written under a superseded schema, is reported as invalid evidence rather than aborting the report path. Only a slot's current publication is evidence: an older identity in the same slot is diagnostic history, so an unreadable historical manifest is logged and skipped, while an unreadable manifest that the slot's `current.json` names — or one in a slot with no readable pointer — is reported as invalid evidence and blocks the report.
...
| `attacks_and_boundaries.hidden_source_backdoor.poison_fraction_sweep` | the admissible source-backdoor poison fractions: the confirmatory fraction is checked against it when the configuration loads, and any backdoor training step declaring a fraction outside the sweep is rejected |
...
A workspace that still holds artifacts written under a superseded manifest schema cannot pass report verification until those slots are republished or cleared, because such a slot's current publication is unreadable. The prerecorded prerequisite for the scientific run is therefore a `preprocess` pass that (re)publishes every prepared view's provenance, followed by clearing any artifact workspace left by an earlier schema rather than reusing it.
