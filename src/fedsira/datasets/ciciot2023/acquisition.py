...
def compute_dataset_manifest_hash(discovered: Sequence[SecondaryCsvFile]) -> ArtifactDigest:
    ...
    canonical_manifest = tuple(
        (item.relative_path, item.file_sha256) for item in sorted(discovered, key=lambda item: item.relative_path)
    )
    return hashlib.sha256(canonical_bytes("CICIOT2023_DATASET_MANIFEST_V1", canonical_manifest)).hexdigest()
