...
from fedsira.domain.enums import AlgorithmName, SeedDerivationLabel
...
def test_namespace_seed_matches_manual_reference_computation() -> None:
    master_seed = 1103
    namespace = SeedDerivationLabel.DATA_SPLIT
...
def test_namespace_seed_is_deterministic() -> None:
    first = namespace_seed(1217, SeedDerivationLabel.MODEL_INITIALIZATION)
    second = namespace_seed(1217, SeedDerivationLabel.MODEL_INITIALIZATION)
    assert first == second


def test_namespace_seed_differs_across_namespaces() -> None:
    first = namespace_seed(1103, SeedDerivationLabel.DATA_SPLIT)
    second = namespace_seed(1103, SeedDerivationLabel.DOMAIN_PARTITION)
    assert first != second


def test_namespace_seed_differs_across_master_seeds() -> None:
    first = namespace_seed(1103, SeedDerivationLabel.DATA_SPLIT)
    second = namespace_seed(1217, SeedDerivationLabel.DATA_SPLIT)
    assert first != second
...
def test_derive_uint32_is_deterministic() -> None:
    first = derive_uint32(SeedDerivationLabel.LOCAL_TRAINING_JOB, 1, "a", "b", 3)
    second = derive_uint32(SeedDerivationLabel.LOCAL_TRAINING_JOB, 1, "a", "b", 3)
    assert first == second


def test_derive_uint32_changes_with_any_field() -> None:
    baseline = derive_uint32(SeedDerivationLabel.LOCAL_TRAINING_JOB, 1, "a", "b", 3)
    assert derive_uint32(SeedDerivationLabel.LOCAL_TRAINING_JOB, 2, "a", "b", 3) != baseline
    assert derive_uint32(SeedDerivationLabel.LOCAL_TRAINING_JOB, 1, "a", "b", 4) != baseline
    assert derive_uint32(SeedDerivationLabel.COMMITTEE_DRAW, 1, "a", "b", 3) != baseline


def test_local_training_seed_is_deterministic() -> None:
    first = local_training_seed(
        42, "a" * 64, "checkpoint-1", AlgorithmName.ANCHOR_FEDAVG, "domain-1", "clean", -1
    )
    second = local_training_seed(
        42, "a" * 64, "checkpoint-1", AlgorithmName.ANCHOR_FEDAVG, "domain-1", "clean", -1
    )
    assert first == second
...
def test_deterministic_order_is_a_stable_permutation() -> None:
    ...
    ordered_once = deterministic_order(items, SeedDerivationLabel.SOURCE_SELECTION, 7)
    ordered_twice = deterministic_order(items, SeedDerivationLabel.SOURCE_SELECTION, 7)
...
def test_deterministic_order_changes_with_namespace_seed() -> None:
    ...
    ordered_a = deterministic_order(items, SeedDerivationLabel.SOURCE_SELECTION, 1)
    ordered_b = deterministic_order(items, SeedDerivationLabel.SOURCE_SELECTION, 2)
