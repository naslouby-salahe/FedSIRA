...
EVIDENCE_MINIMA = CAPABILITY_CONTRACT_CONFIG.evidence_minima
...
def test_build_capability_contract_uses_dataset_scope_and_configured_thresholds() -> None:
    contract = _contract()
    assert contract.target_class == NBaiotClass.GAFGYT_COMBO.value
    assert contract.supported_class_count == SUPPORTED_CLASS_COUNT
    assert contract.target_f1_minimum == CAPABILITY_CONTRACT_CONFIG.target_f1_minimum
    assert (
...
def test_compute_capability_identity_is_deterministic() -> None:
    first = compute_capability_identity(_contract())
    second = compute_capability_identity(_contract())
    assert first == second
    assert len(first) == 64


def test_compute_capability_identity_changes_when_dataset_manifest_hash_changes() -> None:
    baseline = compute_capability_identity(_contract())
    changed = compute_capability_identity(_contract(dataset_manifest_hash="c" * 64))
    assert baseline != changed


def test_compute_capability_identity_changes_when_scope_changes() -> None:
    baseline = compute_capability_identity(_contract())
    changed = compute_capability_identity(_contract(feature_schema_hash="d" * 64))
    assert baseline != changed
...
def test_capability_contract_passes_requires_both_gamma_and_beta() -> None:
    contract = _contract()
    passing = capability_contract_passes(
        contract,
    ...
    assert passing
    failing_gamma = capability_contract_passes(
        contract,
    ...
    failing_beta = capability_contract_passes(
        contract,
        MetricResult(value=contract.target_f1_minimum, denominator=10),
        MetricResult(value=contract.target_f1_gain_over_anchor_minimum, denominator=10),
...
def test_capability_contract_passes_na_metric_is_not_passing() -> None:
    contract = _contract()
    assert not capability_contract_passes(
        contract,
