...
from fedsira.domain.enums import AdmissionOpeningMode, AdmissionState
...
ADMISSION_OPENING_CONFIG = CONFIG.protocol.admission_opening
...
def test_start_admission_proposal_assisted_commits_source_with_zero_weight() -> None:
    entry = start_admission(AdmissionOpeningMode.PROPOSAL_ASSISTED)
    assert entry.state is AdmissionState.CANDIDATE_SCREEN
    assert entry.source_committed is True
    assert entry.direct_production_weight == 0.0


def test_start_admission_candidate_free_does_not_commit_source() -> None:
    entry = start_admission(AdmissionOpeningMode.CANDIDATE_FREE)
...
def test_candidate_screen_fewer_than_required_adequate_domains_is_dormant() -> None:
    ...
    state = candidate_screen_transition(
        AdmissionOpeningMode.PROPOSAL_ASSISTED, results, ADMISSION_OPENING_CONFIG
    )
...
def test_candidate_screen_proposal_mode_enough_positive_opens_claim() -> None:
    ...
    assert state is AdmissionState.ADMISSION_OPEN
...
def test_candidate_screen_candidate_free_mode_enough_hard_domains_opens_claim() -> None:
    results = _results([True, True, True], [True, True, False])
    state = candidate_screen_transition(
        AdmissionOpeningMode.CANDIDATE_FREE, results, ADMISSION_OPENING_CONFIG
    )
    assert state is AdmissionState.ADMISSION_OPEN
...
def test_screen_domain_order_is_deterministic_and_takes_the_configured_prefix() -> None:
    ...
    first = screen_domain_order(eligible, 42, ADMISSION_OPENING_CONFIG.screen_domains)
    second = screen_domain_order(eligible, 42, ADMISSION_OPENING_CONFIG.screen_domains)
    assert first == second
    assert len(first) == ADMISSION_OPENING_CONFIG.screen_domains
...
def test_screen_domain_decision_is_positive_requires_all_four_predicates() -> None:
    passing = screen_domain_decision_is_positive(
        ...
        MetricResult(
            value=CAPABILITY_CONTRACT_CONFIG.target_f1_gain_over_anchor_minimum, denominator=10
        ),
        MetricResult(
            value=CAPABILITY_CONTRACT_CONFIG.supported_macro_f1_drop_maximum, denominator=10
        ),
        MetricResult(
            value=CAPABILITY_CONTRACT_CONFIG.benign_false_alarm_rate_increase_maximum,
            denominator=10,
        ),
        PROPOSAL_SCREEN_CONFIG,
        CAPABILITY_CONTRACT_CONFIG,
    )
    assert passing
...
def test_raw_target_f1_screen_domain_decision_fails_when_target_gain_too_small() -> None:
    result = raw_target_f1_screen_domain_decision_is_positive(
        MetricResult(
            value=CAPABILITY_CONTRACT_CONFIG.target_f1_gain_over_anchor_minimum - 0.001,
            denominator=10,
        ),
        MetricResult(value=0.0, denominator=10),
        MetricResult(value=0.0, denominator=10),
...
def test_candidate_free_screen_domain_predicate_boundary() -> None:
    maximum = CAPABILITY_CONTRACT_CONFIG.candidate_free_anchor_target_f1_maximum
    assert candidate_free_screen_domain_predicate(
        MetricResult(value=maximum - 0.01, denominator=10), CAPABILITY_CONTRACT_CONFIG
    )
    assert not candidate_free_screen_domain_predicate(
        MetricResult(value=maximum, denominator=10), CAPABILITY_CONTRACT_CONFIG
    )
    assert not candidate_free_screen_domain_predicate(
        MetricResult(value=None, denominator=0), CAPABILITY_CONTRACT_CONFIG
    )
