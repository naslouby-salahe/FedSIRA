...
def _client_data(seed: int) -> tuple[torch.Tensor, torch.Tensor, tuple[str, ...], int]:
...
def test_run_anchor_fedavg_training_requires_exactly_the_configured_round_count() -> None:
    initial_state_dict = FedSIRAClassifier(4, 2).state_dict()
...
def test_run_anchor_fedavg_training_returns_a_checkpoint_per_round() -> None:
    initial_state_dict = FedSIRAClassifier(4, 2).state_dict()
    rounds = [[_client_data(1), _client_data(2)] for _ in range(ANCHOR_CONFIG.rounds)]
