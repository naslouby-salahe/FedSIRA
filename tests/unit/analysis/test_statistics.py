...
def test_holm_adjusted_p_values_is_monotonic_nondecreasing_in_rank_order() -> None:
    ...
    adjusted = holm_adjusted_p_values(raw)
    values = [value for _, value in adjusted]
    assert values == sorted(values)


def test_holm_adjusted_p_values_ties_broken_by_ascending_canonical_name() -> None:
    raw = [("z", 0.02), ("a", 0.02)]
    adjusted = holm_adjusted_p_values(raw)
    assert [name for name, _ in adjusted] == ["a", "z"]
